...
from libnetfilter_log import libnflog, _LP_nflog_handle, _LP_nflog_g_handle, \
    _LP_nflog_data, nflog_callback
from lib.netlink.netlink_wrapper import nlif_handle
...
class nflog_handle(_LP_nflog_handle):
    ...
    @property
    def fd(self):
        self._fd = socket.fromfd(libnflog.nflog_fd(self), socket.AF_NETLINK,
                                 socket.SOCK_DGRAM)
        return self._fd
...
class nflog_data(_LP_nflog_data):
    ...
    def payload(self):
        ...
        if r <= 0:
            return None
        return (ctypes.c_char_p * r).from_address(
